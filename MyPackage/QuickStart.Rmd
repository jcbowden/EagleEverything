...
<p>
Eagle is a software package for genome-wide association mapping in animal, plant, or human populations of arbitary size and structure.  Our method differs from most other association mapping methods in that we fit all marker-trait associations simultaneously, while still identifying the "best" set of marker loci in strongest association with a trait. Those marker loci in strongest and significant association with a trait are found iteratively.  Unlike most other association mapping methods, we avoid the difficultly of having to set genome-wide significance thresholds by  treating association mapping as a model selection problem. 
</p>

<h1><b> Getting Started </b> </h1>
<p>
Eagle is written in  [R](https://www.r-project.org/about.html) and  installed from [CRAN](https://cran.r-project.org) with the R command
```{r eval=FALSE}
install.packages("Eagle")
```

Eagle makes extensive use of implicit and explicit parallelisation to speed computation.  See our Youtube Installation video for details. 

Here, we will assume that you have the latest version of [R](https://www.r-project.org/about.html) and that you have successfully installed Eagle. 
...
<p>
You will see a message saying that Eagle has been loaded. 
</p>

<h1> <b> Running Eagle the Easy Way </b> </h1>
<p>
There are two ways in which the functions in Eagle can be run. One is via the traditional approach of writing an R script with 
the Eagle functions that read in the data and perform the analysis. R users will be familar with this approach. See our "UserManual" vignette  for more details (just 
type vignette("UserManual", "Eagle") at the R prompt). 
</p>
